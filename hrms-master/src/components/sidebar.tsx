...
function Sidebar({ isOpen }: SidebarProps) {
        ...
        const fetchMenuOrder = async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    // The data returned from settings API is directly the value (the array)
    ...
    // Filter dynamic items by permissions
    const filteredMenuItems = dynamicMenuItems.filter(item => {
        if (!user?.permissions) return true;

        // If permissions object is empty/null, show all (Super Admin behavior)
            ...
            <div className="sidebar-content-wrapper">
                ...
                <div className="sidebar-header">
                    <div className="brand-container">
                        <img src={logo} alt="MineHR Logo" className="brand-icon" />
                    </div>

                    <div className="user-profile">
                        <div className="user-detail">
                            <Building2 size={20} className="profile-icon" />
                            <span className="company-text">MineHR-Solutions Pvt. Ltd.</span>
                        </div>
                    </div>
                </div>
