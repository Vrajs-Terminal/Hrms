...
const OvertimeApproval = () => {
    const { requests, loading, reviewRequest } = useAttendanceRequests('Overtime');
    ...
    return (
        <div className="attendance-module-container">
            <div className="attendance-header">
                <div><h2 className="attendance-title">Overtime Approval</h2><p className="attendance-subtitle">Approve or reject overtime requests.</p></div>
            </div>
                        ...
                        {loading ? <tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px', color: '#64748b' }}>Loading...</td></tr>
                            : filtered.length > 0 ? filtered.map((r: any) => (
                                <tr key={r.id}>
                                    ...
                                    <td>{formatDate(r.date)}</td><td><Clock size={12} /> {r.requested_data?.hours || '--'}h</td><td style={{ fontSize: 12, color: '#475569' }}>{r.reason}</td><td><span className={`badge ${getStatusBadge(r.status)}`}>{r.status}</span></td>
                                    <td>{r.status === 'Pending' ? (activeId === r.id ? (<div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}><input type="text" placeholder="Remarks..." value={remarks} onChange={e => setRemarks(e.target.value)} style={{ padding: '6px 10px', fontSize: 12, borderRadius: 6, border: '1px solid #e2e8f0' }} /><div style={{ display: 'flex', gap: 6 }}><button className="btn-primary" style={{ padding: '4px 12px', fontSize: 11 }} onClick={() => handleReview(r.id, 'Approved')}><CheckCircle size={12} /> Approve</button><button className="btn-secondary" style={{ padding: '4px 12px', fontSize: 11, color: '#ef4444' }} onClick={() => handleReview(r.id, 'Rejected')}><XCircle size={12} /> Reject</button></div></div>) : (<button className="btn-secondary" style={{ padding: '4px 12px', fontSize: 11 }} onClick={() => setActiveId(r.id)}>Review</button>)) : <span style={{ fontSize: 11, color: '#94a3b8' }}>{r.approver?.name ? `By ${r.approver.name}` : '--'}</span>}</td>
                                </tr>
                            )) : <tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px', color: '#64748b' }}>No overtime requests to review.</td></tr>}
                    </tbody>
                </table>
            </div>
        </div>
